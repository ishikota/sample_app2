  ...
  describe "authorization" do
    describe "for non-signed-in users" do
      ...
      describe "in the Relationships controller" do
       ...
       describe "submitting to the destrpu action" do
    ...
    describe "as wrong user" do
      let(:user) { FactoryGirl.create(:user) }
      let(:wrong_user) { FactoryGirl.create(:user, email: "wrong@co.jp") }
      before { sign_in user, no_capybara: true }

      describe "submitting GET request to edit action" do
        before { get edit_user_path(wrong_user) }
        specify { expect(response.body).not_to match(full_title('Edit user')) }
        specify { expect(response).to redirect_to(root_path) }
      end

      describe "submitting PATCH request to update action" do
        before { patch user_path(wrong_user) }
        specify { expect(request).to redirect_to(root_path) }
      end
    end
